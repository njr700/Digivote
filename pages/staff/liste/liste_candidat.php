...
// Initialisation du tableau de cartes d'identité
$_SESSION['userCards'] = [];
if ($result->rowCount() > 0) {
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        $_SESSION['userCards'][] = [
            'profilePicture' => htmlspecialchars($row['photo']),
            'nom' => htmlspecialchars($row['prenom']),
            'prenom' => htmlspecialchars($row['nom']),
            'postnom' => htmlspecialchars($row['postnom']),
            'poste' => htmlspecialchars($row['poste']),
            'numero_identite' => htmlspecialchars($row['numero_identite'])
        ];
    }
}
