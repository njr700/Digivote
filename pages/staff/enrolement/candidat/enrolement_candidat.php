...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $postnom = htmlspecialchars($_POST['postnom']);// Notez le changement ici pour correspondre à l'entrée HTML
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["profilePicture"]["tmp_name"], $target_file)) {
            ...
            try {
                $stmt = $bdd->prepare("INSERT INTO candidat (nom, prenom, postnom, poste, numero_identite, photo, defaultPassword) VALUES (:nom, :prenom, :postnom, :poste, :numero_identite, :photo, :defaultPassword)");
                $stmt->bindParam(':nom', $nom);
                $stmt->bindParam(':prenom', $prenom);
                $stmt->bindParam(':postnom', $postnom);// Notez le changement ici pour correspondre à l'entrée HTML
